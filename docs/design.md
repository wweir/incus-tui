...
**总结**：Web UI 是“CLI 功能的图形封装”，高度依赖 Incus REST API。TUI 的目标是提供**终端原生、键盘友好、支持基础鼠标交互、无浏览器依赖** 的等价体验，尤其适合服务器/无头环境。
...
- TUI 可实现“键盘驱动 + 鼠标辅助选择 + 实时交互 + 表格/树形视图”，结合 CLI 的全部能力，提供类似 `htop` / `lazygit` / `k9s` 的沉浸式体验。
- 适用于运维、开发、边缘部署场景。
...
每个模块均支持**键盘快捷键**（类似 vim/k9s：j/k 上下、Enter 详情、/ 搜索、? 帮助），并支持鼠标滚轮/点击完成基础表格选择。
...
- **菜单写操作建模**：非实例资源按菜单项定义表单字段，再映射到对应 Incus API 请求；不再用单个 `value` 字段覆盖所有资源。
- **更新语义**：对不易直接预填当前值的 update 表单使用 `keep` 语义，避免误清空未知字段。
- **数据流**：
  - 异步查询（`tea.Cmd`）避免阻塞 UI。
  - 实时更新：使用 `incus monitor` 或 WebSocket-like polling。
- **认证与安全**：完全复用 Incus CLI 逻辑（Unix socket 优先，HTTPS + TLS 证书）。
- **本地权限处理**：启动阶段对默认本地 Unix socket 做一次访问探测；若为权限拒绝，则自动走一次 `sudo` 重启，不对显式远端做提权。
